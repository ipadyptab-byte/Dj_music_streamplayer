...
def get_audio_url(video_url):
    ...
    try:
        ...
        print('yt-dlp executable fallback failed:', e)</old_code><new_code># ===============================
# Routes
# ===============================
@app.route('/')
def index():
...
if __name__ == '__main__':
    ...
    app.run(host='127.0.0.1', port=5000')c_file('index.html')
...
def play():
    ...
    if audio_url:
        return jsonify({'url': audio_url})
    else_stati</old_code><new_code>@app.route('/api/search')
def search():
    query = request.args.get('q')
    source = request.args.get('source', 'youtube')
    return jsonify(search_media(source, query)) if query else jsonify([])
