...
class PlaybackController:
    ...
    This effectively "pauses/stops" whatever was playing before.
    """

    def __init__(self) -> None:
        self.lock = threading.Lock()
        self.proc: subprocess.Popen | None = None
        self.kind: str | None = None  # "main", "ad", "prayer"

    def play(self, path: str, kind: str) -> None:
        """Start playing `path` as the given kind, stopping any existing audio."""
        with self.lock:
...
def ad_worker(state: SchedulerState, ui: "MainWindow") -> None:
    ...
    while state.running:
        ...
        if ad_file and os.path.exists(ad_file):
            ...
            # This will stop any currently playing MAIN track
            play_with_ffplay(ad_file, kind="ad")


def prayer_worker(state: SchedulerState, ui: "MainWindow") -> None:
    """Check every second and play prayer at configured times.

    Prayer has highest priority: it stops any ad/main currently playing.
    """
    while state.running:
        time.sleep(1)
        now = datetime.now()
        ...
        for t in times:
            if t == current_hm and last_run.get(t) != today:
                if os.path.exists(prayer_file):
                    ui.log(
                        f"Playing prayer for scheduled time {t}: "
                        f"{os.path.basename(prayer_file)}"
                    )
                    # Mark that we've run this time today
                    with state.lock:
                        state.prayer_last_run[t] = today

                    # This will stop ANY currently playing audio (main or ad)
                    play_with_ffplay(prayer_file, kind="prayer")
...
class MainWindow:
    def __init__(self, root: tk.Tk, state: SchedulerState) -> None:
        self.root = root
        self.state = state
        self.root.title("Headless Music Scheduler (Ad + Prayer + Search)")

        # --- Search & play section (MAIN track) ---
        search_frame = tk.LabelFrame(
            root, text="Search & Play (YouTube via yt-dlp)", padx=10, pady=10
        )
        ...
        btn_ad_select = tk.Button(
            ad_frame, text="Select Advertisement Track", command=self.select_ad_track
        )
        ...
        btn_apply_interval = tk.Button(
            ad_frame, text="Apply Interval", command=self.apply_interval
        )
        ...
        btn_prayer_select = tk.Button(
            prayer_frame, text="Select Prayer Track", command=self.select_prayer_track
        )
        ...
        btn_remove_time = tk.Button(
            btns_frame, text="Remove Selected", command=self.remove_selected_time
        )
    ...
    def apply_interval(self) -> None:
        try:
            ...
            messagebox.showerror(
                "Invalid interval",
                "Please enter a positive integer number of seconds.",
            )
            return
        with self.state.lock:
    ...
    def add_time(self) -> None:
        def on_ok() -> None:
            ...
            try:
                ...
                messagebox.showerror(
                    "Invalid time",
                    "Please enter time in 24h format HH:MM, e.g. 06:30",
                )
                return
    ...
    def play_search_result(self, track: dict) -> None:
        ...
        if not audio_rel:
            messagebox.showerror(
                "Error", "Failed to resolve audio URL (yt-dlp error)."
            )
            return
        ...
        self.log(f"Playing search result (main track): {title}")
        # MAIN playback: this will be stopped when an Ad or Prayer starts
        play_with_ffplay(path, kind="main")
...
def main() -> None:
    ...
    ui.log("Application started. Configure advertisement, prayer, and search.")
